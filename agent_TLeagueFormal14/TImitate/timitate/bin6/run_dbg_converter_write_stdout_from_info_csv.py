"""Run dbg, each worker writes stdout to file."""
import logging
import os
from os import path
import subprocess
...
flags.DEFINE_string("output_dir", '/root/dbgyymmddmmss',
                    "dir where the content goes.")
...
class MyResult(object):
  def __init__(self, replay_name, player_id, game_version):
...
def main_worker():
  ...
  def _dbg_converter(replay_name, player_id, game_version):
    ...
    cmd = [
      'python3', '-m', 'timitate.bin6.debug_converters_v3',
    ...
    basename = path.join(FLAGS.output_dir,
                         '{}-{}'.format(replay_name, player_id))
    log_file = open(basename + '.log', 'w')
    err_file = open(basename + '.err', 'w')
    log_file.write('replay_name: {}, player_id: {}, game_version: {}\n'.format(
      replay_name, player_id, game_version
    ))
    log_file.flush()
    # our sc2 bin path
    ...
    return MyResult(replay_name, player_id, game_version)
