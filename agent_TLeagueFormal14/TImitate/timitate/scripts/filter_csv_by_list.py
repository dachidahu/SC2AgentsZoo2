"""filter a csv file by a list and generate a new csv file"""
import csv
import random
...
flags.DEFINE_string("input_csv_path", '/csv/input/path', "input csv path",
                    short_name='i')
flags.DEFINE_string("input_list_path", '/input/list/path', "input list path",
                    short_name='s')
...
def main(__):
  ...
  input_list_path = FLAGS.input_list_path
  ...
  # create replay-player_id set
  with open(input_list_path, 'rt') as f:
    lines = f.readlines()
  ...
  for i, line in enumerate(lines):
    tmp = '{}-{}'.format(line['replay_name'], line['player_id'])
    if tmp not in rep_ids:
      continue
